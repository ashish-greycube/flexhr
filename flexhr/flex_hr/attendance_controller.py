...
from frappe.utils import add_days, cint, cstr, flt, getdate, rounded, date_diff, money_in_words,time_diff_in_seconds,today
...
def run_job(start_date,end_date):
    try:
        ...
        process_employee_checkin_records(start_date,end_date,att_log)
		...
		# notify_errors(err_msg,att_log.name)
	#finally:
		#att_log.save(ignore_permissions=True)
		...
		if check_in:
			for emp in check_in:
				...
				if holiday==True:
					...
					remark='Holiday. Attendance device data is not processed'
					action=''
					review=False
					reviewer=None			
					...
					remark='Employee is absent on '+ dt +' as per device'
					...
					if att_status == 'On Leave' or att_status == 'Half Day':
						...
						remark='Working day. Employee is absent and there is leave record '+ str(leave_name)
						action=''
						review=False
						reviewer=None
					else:
						# case that needs to be solved by HR
						remark='Working day. Employee is absent. Leave record '+ str(leave_name) + '.Attendance record '+att_name+' with status '+att_status
					...
					if att_detail!=None and (att_status == 'Present' or att_status == 'Half Day'):
						...
						remark='Attendance record '+str(att_name)+ ' with status '+ str(att_status)
						if att_status == 'Present':
							remark='Rerun, Attendance record '+str(att_name)+ ' exists with in '+ str(emp_in_time) + ' out '+ str(emp_out_time)
						action=''
						review=False
						reviewer=None
					else:
					...
					if att_detail!=None and (att_status == 'Present' or att_status == 'Half Day'):
						...
						if att_detail==None:
							#Employee is not fully present and there is no leave, so we create LWP
							...
							review_remark='Leave is update by system'
							update_attendance_remark(new_att_name,review_remark)
							action='[A] Either only IN or only OUT time is present and hence absent, so LWP '+leave_name+' is created OR [B] Cancel leave '+leave_name+' to give present'
							review=False
						elif att_detail!=None and att_status =='On Leave':
							#Employee is not fully present and there is leave, so we just update leave description
							if (att_review_remark=='Leave is update by system'):
								# it is rerun so do nothing
								remark='Rerun. Missing in/out and leave is already updated'
								review=False
								reviewer=None
							else:							
...
def notify_errors(exceptions,att_log):
	print att_log
...
def split_multiple_leaves(salary_start_date,salary_end_date):
	...
	# validate_employee_leave_on_salary_boundary(salary_end_date)

def split_leave(leave_name,split_date,split_type='one_part'):
	print leave_name,split_date,split_type
	leave=frappe.get_doc('Leave Application',leave_name)
	print leave
	if leave:
		first_half_day=None
		...
		if split_type=='one_part':
			...
			if leave.half_day_date:
				if leave.half_day_date<=split_date:
			...
			print second_total_leave_days

			if first_total_leave_days>0 or second_total_leave_days>0: 
				leave.cancel()
		if (split_type=='one_part' and first_total_leave_days > 0) or (split_type=='two_part' and first_total_leave_days > 0):
			...
			new_leave.description='Split of '+leave_name
		...
		if split_type=='two_part' and second_total_leave_days > 0:
			...
			new_leave.status='Approved'
			new_leave.posting_date=today()
			new_leave.description='Split 2nd of '+leave_name
			new_leave.save(ignore_permissions=True)
			new_leave.submit()
